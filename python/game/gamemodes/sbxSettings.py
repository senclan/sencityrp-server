...
class sbxSettingsManager:
	def __init__(self):
		self.mpObjectLimit = 2000
		self.hpConstructionHealth = 150
	...
	def loadGameSettings(self, file="/settings/sandbox/game.ini"):
		...
		try:
			...
			if hpZombie > 100: hpZombie = 100
			if hpConstruction > self.hpConstructionHealth: hpConstruction = self.hpConstructionHealth
			if hpCombat > 100: hpCombat = 100
		except: ExceptionOutput()
		try:
